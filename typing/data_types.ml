...
let equal_tag t1 t2 =
  ...
  | Cstr_extension (path1, _, _), Cstr_extension (path2, _, _)
  | Cstr_rebind (_, path1, _), Cstr_rebind (_, path2, _) ->
      Path.same path1 path2
  | (Cstr_constant _|Cstr_block _|Cstr_unboxed|Cstr_extension _|Cstr_rebind _), _ -> false
